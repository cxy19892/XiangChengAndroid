...
public class SignInCircle extends View{
	...
	public SignInCircle(Context context, AttributeSet attrs) {
		super(context, attrs);
		initView();
	}
	
	public SignInCircle(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	public void setData(long sleepTime, long wakeTime, String dateStr, final float per){
	...
	protected void onDraw(Canvas canvas) {
	    ...
	    if(isBindPillow){ //绑定了硬件，还未同步数据
	    	if(!isSync){
			    ...
			    canvas.drawText("睡眠效率", (oval.right - oval.left) * 1/2 + oval.left - 3.0f * textSize, oval.top + (oval.bottom - oval.top) * 1/3, mPaint);
