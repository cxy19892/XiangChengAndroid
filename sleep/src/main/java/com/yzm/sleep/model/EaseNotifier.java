...
import com.yzm.sleep.utils.EaseUI.EaseSettingsProvider;
...
public class EaseNotifier {
	...
	protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        try {
        	...
        	String notifyText=json.getString("name");
            ...
            if(AppManager.getAppManager().getActivitySize() <=0 ){
            	...
            	msgIntent.putExtra("chattoMsgList", true);
            }else{
            	//按Home键后台运行
            	String currentAName=AppManager.getAppManager().currentActivity().getClass().getSimpleName();
            	if(ChatActivity.class.getSimpleName().equals(currentAName) && fromUsersNum <= 1){
            		msgIntent=new Intent(appContext,ChatActivity.class);
            		UserMessageBean bean=new UserMessageBean();
            		try {
            			JSONObject jsonObj=message.getJSONObjectAttribute("attr");
            			bean.setUid(message.getFrom());
            			bean.setNickname(jsonObj.getString("name"));
            			bean.setProfile(jsonObj.getString("url"));
            			bean.setProfile_key(jsonObj.getString("key"));
            			bean.setIs_zj(jsonObj.getString("type"));
            		} catch (Exception e) {}
            		
            		msgIntent.putExtra("userBean", bean);
            		msgIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
            	}else{
            		msgIntent=new Intent(appContext,HomeActivity.class);
            		msgIntent.putExtra("close_except_homeactivity", true);
            		msgIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
            	}
            }
