...
public class DFUManager {
	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		/*
		 * callback for connection and disconnection		(non-Javadoc)
		...
		/*
		 * callback for service discovery(non-Javadoc)
		...
		public void onServicesDiscovered(BluetoothGatt gatt, int status) {
			...
			if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				for (BluetoothGattService service : services) {
					log(TAG, "+services.size="+services.size()+";Found Service: " + service.getUuid());
					if (service.getUuid().equals(DFU_SERVICE_UUID)) {
						log(TAG, "DFU Service found!");
				...
				if (isDFUServiceFound) {
					...
					mCallbacks.onDFUServiceNotFound();
				}
			} else {
		...
		/*
		 * Callback that tell the status of characteristic write(non-Javadoc) 
		 * if status is BluetoothGatt.GATT_SUCCESS it means characteristic is successfully written on device 
		...
		public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
			...
			if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				if (characteristic.getUuid().equals(DFU_CONTROLPOINT_CHARACTERISTIC_UUID) && !isFileSizeWritten) {
					log(TAG, "successfully written startDFU and now writing file size");
					writeFileSize();
					...
					log(TAG, "successfully written Packet received notification and now writing receive firmware image");
					...
					log(TAG, "successfully written ReceiveFirmwareImage and now writing file");
					startUploadingFile();
					isReceiveFirmwareImageWritten = true;
				}
			} else {
				logE(TAG, ERROR_WRITE_CHARACTERISTIC + " [" + characteristic.getUuid() + "] Error code: " + status);
				mCallbacks.onError(ERROR_WRITE_CHARACTERISTIC, status);
			}
		}

		/*
		 * callback after receiving notification (non-Javadoc)
		...
		*/
		...
		public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
			...
			if (opCode == RECEIVED_OPCODE && request == START_DFU) {
				log(TAG, "Received notification for StartDFU");
				if (characteristic.getValue()[2] == OPERATION_SUCCESS) {
					log(TAG, "File length is valid: " + characteristic.getValue()[2]);
					enablePacketNotification();
				} else {
					errorStatus = characteristic.getValue()[2];
					logE(TAG, ERROR_FILE_LENGTH + " (" + errorStatus + ")");
			...
			//here we transfer next packet of 20 bytes
				...
				log(TAG, "Received Notification for sent Packet");
				...
				log(TAG, "Bytes received in Packet: " + receivedBytes);
				mCallbacks.onFileTranfering(receivedBytes);
				if (!isLastPacket && !mStopSendingPacket) {
					sendPacket();
				} else {
					log(TAG, "last packet notification received");
				}
			}
			...
			// if file has been transfered successfully the we will validate transfered file 
				...
				log(TAG, "File has been transfered");
				if (characteristic.getValue()[2] == OPERATION_SUCCESS) {
					log(TAG, "Successful File transfer!");
					mCallbacks.onFileTransferCompleted();
					validateFirmware();
				} else {
					errorStatus = characteristic.getValue()[2];
					logE(TAG, ERROR_FILE_TRANSFER + errorStatus);
			...
			//if file has been validated successfully then we will activate and reset device 
				...
				log(TAG, "Transfered file has been validated");
				if (characteristic.getValue()[2] == OPERATION_SUCCESS) {
					log(TAG, "Successful File Transfer Validation!");
					mCallbacks.onFileTransferValidation();
					activateAndReset();
					isNotificationEnable = false;
				} else {
					errorStatus = characteristic.getValue()[2];
					logE(TAG, ERROR_FILE_VALIDATION + errorStatus);
		...
		/*
		 * callback after writing on descriptor for enabling and disabling notification (non-Javadoc)
		...
		public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
			if (status == BluetoothGatt.GATT_SUCCESS) {
				if (isNotificationEnable) {
					...
					log(TAG, "Notification is disabled!");
				}
			} else {
	...
	private void writeFileSize() {
		...
		if (isDFUServiceFound) {
			...
			logE(TAG, "writing File size"+mFileSize);
	...
	public void openFile(InputStream stream) {
		try {
			...
			log(TAG, "File Size: " + mFileSize);
		} catch (IOException e) {
			logE(TAG, ERROR_FILE_OPEN + " " + e);
	...
	public void closeFile() {
		if (mFileStream != null) {
			try {
				...
				logE(TAG, ERROR_FILE_CLOSE + " " + e.toString());
	...
	private void sendPacket() {
		...
		if (mPacketNumber == mTotalPackets) {
			...
			log(TAG, "sendPacket: " + mPacketNumber);
			isLastPacket = true;
	...
	/*
	 * systemReset will reset device in DFU mode if something goes wrong during uploading
	 * so that uploading can be started from the beginning
	 */
	public void systemReset() {
		byte[] value = { SYSTEM_RESET };
		mDFUControlPointCharacteristic.setValue(value);
		mBluetoothGatt.writeCharacteristic(mDFUControlPointCharacteristic);
	}
